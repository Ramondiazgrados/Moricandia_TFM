---
title: "Impacto de la planta ruderal Moricandia arvensis sobre otras dos especies de Moricandia (M.rytidocarpoides y M.moricandioides"
author: "Ramón Diaz"
date: ""
output:
  html_document:
    toc: true
    number_sections: true
  pdf_document: default
  word_document: default
---
```{r}
knitr::opts_chunk$set(echo = TRUE)
options(Encoding="UTF-8")
options(tikzDefaultEngine = "xetex")
```

##Cargamos los paquetes necesarios
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
library(readxl)
library(dplyr)
library(ggplot2)
library(calibrate)
library(MASS)
library(DataExplorer)
library(lmPerm)
```

##Cargamos los datos 

#Datos de biomasa

```{r}
dat_bm<- read_excel("Moricandia_BIOMASA_bueno.xlsx")
View(dat_bm[1:10,])

dat_bm<-dat_bm[!(apply(dat_bm, 1, function(y) any(y == 0))),]
dat_bm[dat_bm==0] <- NA

dat_bm<-na.omit(dat_bm)

```

#Datos de supervivencia

```{r}
dat_sup<- read_excel("Moricandia.xlsx", 
     col_types = c("numeric", "text", "numeric", 
         "text", "text", "text", "text", "text", 
         "numeric", "numeric", "numeric", 
         "numeric", "numeric", "numeric", 
         "numeric", "numeric", "numeric", 
         "numeric", "numeric", "numeric", 
         "numeric", "numeric", "numeric", 
         "numeric","numeric","numeric"))
View(dat_sup[1:10,])

```
##Análisis exploratorio 

Podríamos realizar un análisis explortatorio de las variables que tenemos. (Esto podría obviarlo, porque tampoco creo que sirva mucho a parte de ver un poco la distribución de los datos)

# Análisis exploratorio de los datos de biomasa
```{r}
plot_str(dat_bm)
plot_histogram(dat_bm)
plot_bar(dat_bm)
plot_density(dat_bm)
plot_correlation(dat_bm, type = 'continuous','Review.Date')

```


# Análisis exploratorio de los datos de supervivencia
```{r}
plot_str(dat_sup)
plot_histogram(dat_sup)
plot_bar(dat_sup)
plot_density(dat_sup)
```

##Comparación de tratamientos con lmperm
#Subset de datos
Vamos a escoger solo datos de Moricandia arvensis
```{r}
dat_bm_ar<-subset(dat_bm, dat_bm$Especie=="M.arvensis")

dat_bm_ar_q <-subset(dat_bm_ar, dat_bm_ar$Localidad=="Quesada")
range(dat_bm_ar_q$`Biomasa(g)`)
summary(dat_bm_ar_q)

dat_bm_ar_b <-subset(dat_bm_ar, dat_bm_ar$Localidad=="Baza")
range(dat_bm_ar_b$`Biomasa(g)`)
summary(dat_bm_ar_b)
```

Ahora vamos a coger los datos de Moricandia rytidocarpoides

```{r}
dat_bm_ry<- subset(dat_bm, dat_bm$Especie=="M.rytidocarpoides")
range(dat_bm_ry$`Biomasa(g)`)
summary(dat_bm_ry)
```

Por último también cogemos solo los datos de Moricandia moricandioides
```{r}
dat_bm_mo <-subset(dat_bm, dat_bm$Especie=="M.moricandioides")
range(dat_bm_mo$`Biomasa(g)`)
summary(dat_bm_mo)
```

##Análisis de la Biomasa
#M.arvensis (Quesada)
 vamos a ver si Moricandia arvensis (Quesada) tiene diferente comportamiento según los tipos de tratamiento (densidad, suelo y competencia)

```{r}
summary(lmp(dat_bm_ar_q$`Biomasa_g`~dat_bm_ar_q$Suelo*dat_bm_ar_q$Densidad*dat_bm_ar_q$Competencia, perm="Exact"))

```

#M.arvensis (Baza)
En cuanto a Moricandia arvensis (Baza), los resultados de la permanova son: 

```{r}
  summary(lmp(dat_bm_ar_b$`Biomasa_g`~dat_bm_ar_b$Suelo*dat_bm_ar_b$Densidad*dat_bm_ar_b$Competencia, perm="Exact"))
```

¿Qué pasa si añado "Localidad" como factor extra?? (solo por curiosidad)
```{r}
summary(lmp(dat_bm_ar$`Biomasa_g`~dat_bm_ar$Localidad*dat_bm_ar$Densidad*dat_bm_ar$Competencia*dat_bm_ar$Suelo, perm="Exact"))
```

#M.rytidocarpoides
También lo podemos ver para Moricandia rytidocarpoides
```{r}
summary(lmp(dat_bm_ry$`Biomasa_g`~dat_bm_ry$Suelo*dat_bm_ry$Densidad*dat_bm_ry$Competencia, perm="Exact"))
```

#M.moricandioides
Y finalmente para Moricandia moricandioides 

```{r}
summary(lmp(dat_bm_mo$`Biomasa_g`~dat_bm_mo$Suelo*dat_bm_mo$Densidad*dat_bm_mo$Competencia, perm="Exact"))
```

##Gráficos biomasa

Aquí está la función para calcular la media y las desviaciones de los datos para realizar así los gráficos.

```{r}
################ the functin to caclulte mean, se, sd ####################
summarySE<-function(data=NULL, measurevar, groupvars=NULL, na.rm=T,
                    conf.interval=.95, .drop=TRUE) {
  library(plyr)
  
  # New version of length which can handle NA's: if na.rm==T, don't count them
  length2 <- function (x, na.rm=T) {
    if (na.rm) sum(!is.na(x))
    else       length(x)
  }
  
  # This does the summary. For each group's data frame, return a vector with
  # N, mean, and sd
  datac <- ddply(data, groupvars, .drop=.drop,
                 .fun = function(xx, col) {
                   c(N    = length2(xx[[col]], na.rm=na.rm),
                     mean = mean   (xx[[col]], na.rm=na.rm),
                     sd   = sd     (xx[[col]], na.rm=na.rm)
                   )
                 },
                 measurevar
  )
  
  # Rename the "mean" column    
  datac <- rename(datac, c("mean" = measurevar))
  
  datac$se <- datac$sd / sqrt(datac$N)  # Calculate standard error of the mean
  
  # Confidence interval multiplier for standard error
  # Calculate t-statistic for confidence interval: 
  # e.g., if conf.interval is .95, use .975 (above/below), and use df=N-1
  ciMult <- qt(conf.interval/2 + .5, datac$N-1)
  datac$ci <- datac$se * ciMult
  
  return(datac)
}
```

#M.arvensis (Quesada)
PRIMERO LOS GRÁFICOS DE MORICANDIA ARVENSIS EN CONDICIONES DE QUESADA 

1. Biomasa Moricandia arvensis según el tipo de suelo


```{r}
summ1 <- summarySE(dat_bm_ar_q, measurevar = "Biomasa_g", groupvars=c("Suelo")) ##using function below
summ1<- summ1[-5,]
ggplot(data=summ1,aes(x=Suelo,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

2. Moricandia arvensis según el tipo de densidad

```{r}
summ2 <- summarySE(dat_bm_ar_q, measurevar = "Biomasa_g", groupvars=c("Densidad")) ##using function below
summ2<- summ2[-5,]
ggplot(data=summ2,aes(x=Densidad,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


3. Moricandia arvensis según el tipo de competencia
```{r}
summ3 <- summarySE(dat_bm_ar_q, measurevar = "Biomasa_g", groupvars=c("Competencia")) ##using function below
summ3<- summ3[-5,]
ggplot(data=summ3,aes(x=Competencia,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


4. Moricandia arvensis (Q) según el tipo de suelo y densidad

```{r}
summ4 <- summarySE(dat_bm_ar_q, measurevar = "Biomasa_g", groupvars=c("Suelo","Densidad")) ##using function below
summ4<- summ4[-5,]
ggplot(data=summ4,aes(x=Suelo,y=Biomasa_g,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Biomasa de Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.30))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

5. Moricandia arvensis según tipo de suelo y competencia 

```{r}
summ5 <- summarySE(dat_bm_ar_q, measurevar = "Biomasa_g", groupvars=c("Suelo","Competencia")) ##using function below
summ5<- summ5[-5,]
ggplot(data=summ5,aes(x=Suelo,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.73,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

6.Biomasa Moricandia arvensis según la competencia y densidad 


```{r}
summ6 <- summarySE(dat_bm_ar_q, measurevar = "Biomasa_g", groupvars=c("Densidad","Competencia")) ##using function below
summ6<- summ6[-5,]
ggplot(data=summ6,aes(x=Densidad,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.25,0.18), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

#M.arvensis (Baza)
MORICANDIA ARVENSIS EN CONDICIONES DE BAZA

7. Biomasa Moricandia arvensis(B) según el tipo de suelo


```{r}
summ7 <- summarySE(dat_bm_ar_b, measurevar = "Biomasa_g", groupvars=c("Suelo")) ##using function below
summ7<- summ7[-5,]
ggplot(data=summ7,aes(x=Suelo,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

8. Moricandia arvensis (B) según el tipo de densidad

```{r}
summ8 <- summarySE(dat_bm_ar_b, measurevar = "Biomasa_g", groupvars=c("Densidad")) ##using function below
summ8<- summ8[-5,]
ggplot(data=summ8,aes(x=Densidad,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


9. Moricandia arvensis(B) según el tipo de competencia
```{r}
summ9 <- summarySE(dat_bm_ar_b, measurevar = "Biomasa_g", groupvars=c("Competencia")) ##using function below
summ9<- summ9[-5,]
ggplot(data=summ9,aes(x=Competencia,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


10. Moricandia arvensis (B) según el tipo de suelo y densidad

```{r}
summ10 <- summarySE(dat_bm_ar_b, measurevar = "Biomasa_g", groupvars=c("Suelo","Densidad")) ##using function below
summ10<- summ10[-5,]
ggplot(data=summ10,aes(x=Suelo,y=Biomasa_g,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Biomasa de Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.30))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

11. Moricandia arvensis(B) según tipo de suelo y competencia 

```{r}
summ11 <- summarySE(dat_bm_ar_b, measurevar = "Biomasa_g", groupvars=c("Suelo","Competencia")) ##using function below
summ11<- summ11[-5,]
ggplot(data=summ11,aes(x=Suelo,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.75,0.20), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

12.Biomasa Moricandia arvensis(B) según la competencia y densidad 


```{r}
summ12 <- summarySE(dat_bm_ar_b, measurevar = "Biomasa_g", groupvars=c("Densidad","Competencia")) ##using function below
summ12<- summ12[-5,]
ggplot(data=summ12,aes(x=Densidad,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.73,0.80))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

#M.rytidocarpoides
GRÁFICOS DE MORICANDIA RYTIDOCARPOIDES

13. Biomasa Moricandia rytidocarpoides según el tipo de suelo


```{r}
summ13 <- summarySE(dat_bm_ry, measurevar = "Biomasa_g", groupvars=c("Suelo")) ##using function below
summ13<- summ13[-5,]
ggplot(data=summ13,aes(x=Suelo,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

14. Moricandia rytidocarpoides según el tipo de densidad

```{r}
summ14 <- summarySE(dat_bm_ry, measurevar = "Biomasa_g", groupvars=c("Densidad")) ##using function below
summ14<- summ14[-5,]
ggplot(data=summ14,aes(x=Densidad,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


15. Moricandia rytodcarpoides según el tipo de competencia
```{r}
summ15 <- summarySE(dat_bm_ry, measurevar = "Biomasa_g", groupvars=c("Competencia")) ##using function below
summ15<- summ15[-5,]
ggplot(data=summ15,aes(x=Competencia,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


16. Moricandia rtydocarpoides según el tipo de suelo y densidad

```{r}
summ16 <- summarySE(dat_bm_ry, measurevar = "Biomasa_g", groupvars=c("Suelo","Densidad")) ##using function below
summ16<- summ16[-5,]
ggplot(data=summ16,aes(x=Suelo,y=Biomasa_g,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Biomasa Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

17. Moricandia rytidocarpoides según tipo de suelo y competencia 

```{r}
summ17 <- summarySE(dat_bm_ry, measurevar = "Biomasa_g", groupvars=c("Suelo","Competencia")) ##using function below
summ17<- summ17[-5,]
ggplot(data=summ17,aes(x=Suelo,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.28,0.83))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```
18.Biomasa Moricandia rytidocarpoides según la competencia y densidad 


```{r}
summ18 <- summarySE(dat_bm_ry, measurevar = "Biomasa_g", groupvars=c("Densidad","Competencia")) ##using function below
summ18<- summ18[-5,]
ggplot(data=summ18,aes(x=Densidad,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.73,0.80))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

#M. moricandioides
GRÁFICOS DE MORICANDIA MORICANDIOIDES

19. Biomasa Moricandia moricandioides según el tipo de suelo


```{r}
summ19 <- summarySE(dat_bm_mo, measurevar = "Biomasa_g", groupvars=c("Suelo")) ##using function below
summ19<- summ19[-5,]
ggplot(data=summ19,aes(x=Suelo,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

20. Moricandia rytidocarpoides según el tipo de densidad

```{r}
summ20 <- summarySE(dat_bm_mo, measurevar = "Biomasa_g", groupvars=c("Densidad")) ##using function below
summ20<- summ20[-5,]
ggplot(data=summ20,aes(x=Densidad,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


21. Moricandia moricandioides según el tipo de competencia
```{r}
summ21 <- summarySE(dat_bm_mo, measurevar = "Biomasa_g", groupvars=c("Competencia")) ##using function below
summ21<- summ21[-5,]
ggplot(data=summ21,aes(x=Competencia,y=Biomasa_g))+
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


22. Moricandia moricandioides según el tipo de suelo y densidad

```{r}
summ22 <- summarySE(dat_bm_mo, measurevar = "Biomasa_g", groupvars=c("Suelo","Densidad")) ##using function below
summ22<- summ22[-5,]
ggplot(data=summ22,aes(x=Suelo,y=Biomasa_g,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Biomasa de Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

23. Moricandia moricandioides según tipo de suelo y competencia 

```{r}
summ23 <- summarySE(dat_bm_mo, measurevar = "Biomasa_g", groupvars=c("Suelo","Competencia")) ##using function below
summ23<- summ23[-5,]
ggplot(data=summ23,aes(x=Suelo,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.28,0.83), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

24.Biomasa Moricandia moricandioides según la competencia y densidad 


```{r}
summ24 <- summarySE(dat_bm_mo, measurevar = "Biomasa_g", groupvars=c("Densidad","Competencia")) ##using function below
summ24<- summ24[-5,]
ggplot(data=summ24,aes(x=Densidad,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.72,0.86), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```
#Según Localidad (curiosidad)
Podemos hacer gráficos comparando Moricandia arvensis según localicad (Baza o Quesada)

```{r}
summ25 <- summarySE(dat_bm_ar, measurevar = "Biomasa_g", groupvars=c("Localidad","Suelo")) ##using function below
summ25<- summ25[-5,]
ggplot(data=summ25,aes(x=Localidad,y=Biomasa_g,group=Suelo,colour=Suelo))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Suelo"),labels = c("No"="No perturbado", "Si"="Perturbado"))+
  labs(x=c("Localidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Quesada"="Quesada","Baza"="Baza"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.72,0.46), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


```{r}
summ26 <- summarySE(dat_bm_ar, measurevar = "Biomasa_g", groupvars=c("Localidad","Competencia")) ##using function below
summ26<- summ26[-5,]
ggplot(data=summ26,aes(x=Localidad,y=Biomasa_g,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Localidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Quesada"="Quesada","Baza"="Baza"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.72,0.46), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

```{r}
summ27 <- summarySE(dat_bm_ar, measurevar = "Biomasa_g", groupvars=c("Localidad","Densidad")) ##using function below
summ27<- summ27[-5,]
ggplot(data=summ27,aes(x=Localidad,y=Biomasa_g,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Biomasa Moricandia arvensis")+
  geom_errorbar(aes(ymin=Biomasa_g-se,ymax=Biomasa_g +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Localidad"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Quesada"="Quesada","Baza"="Baza"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.15), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

#Análisis de emergencia y supervivencia

#Subset datos

Vamos a escoger solo datos de Moricandia arvensis
```{r}
dat_sup_ar<-subset(dat_sup, dat_sup$Especie=="M.arvensis" | dat_sup$Especie=="M.moricandioides_M.arvensis"|dat_sup$Especie=="M.rytidocarpoides_M.arvensis")

dat_sup_ar_q <-subset(dat_sup_ar, dat_sup_ar$Localidad=="Quesada")
summary(dat_sup_ar_q)

dat_sup_ar_b <-subset(dat_sup_ar, dat_sup_ar$Localidad=="Baza")
summary(dat_sup_ar_b)
```

Ahora vamos a coger los datos de Moricandia rytidocarpoides

```{r}
dat_sup_ry<- subset(dat_sup, dat_sup$Especie=="M.rytidocarpoides"|dat_sup$Especie=="M.rytidocarpoides_M.arvensis")
summary(dat_sup_ry)
```

Por último también cogemos solo los datos de Moricandia moricandioides
```{r}
dat_sup_mo <-subset(dat_sup, dat_sup$Especie=="M.moricandioides"|dat_sup$Especie=="M.moricandioides_M.arvensis")
summary(dat_sup_mo)
plot_missing(dat_sup_mo)
```

#Análisis EMERGENCIA
Vamos a realizar PERMANOVA de la emergencia para ver si hay diferencia según el tipo de suelo, competencia y/o densidad

Moricandia Arvensis (Q)

```{r}

summary(lmp(dat_sup_ar_q$Plant_emerg~dat_sup_ar_q$Suelo*dat_sup_ar_q$Densidad*dat_sup_ar_q$Competencia, perm="Exact"))

```

En cuanto a Moricandia arvensis (Baza), los resultados de la permanova son: 


```{r}
 summary(lmp(dat_sup_ar_b$Plant_emerg~dat_sup_ar_b$Suelo*dat_sup_ar_b$Densidad*dat_sup_ar_b$Competencia, perm="Exact"))

```

¿Qué pasa si añado "Localidad" como factor extra??
```{r}
summary(lmp(dat_sup_ar$Plant_emerg~dat_sup_ar$Localidad*dat_sup_ar$Densidad*dat_sup_ar$Competencia*dat_sup_ar$Suelo, perm="Exact"))
```

También lo podemos ver para Moricandia rytidocarpoides
```{r}
summary(lmp(dat_sup_ry$Plant_emerg~dat_sup_ry$Suelo*dat_sup_ry$Densidad*dat_sup_ry$Competencia, perm="Exact"))
```

Y finalmente para Moricandia moricandioides 

```{r}
summary(lmp(dat_sup_mo$Plant_emerg~dat_sup_mo$Suelo*dat_sup_mo$Densidad*dat_sup_mo$Competencia, perm="Exact"))
```

#Análisis supervivencia

Moricandia Arvensis (Q)

```{r}
summary(lmp(dat_sup_ar_q$Sup_porc~dat_sup_ar_q$Suelo*dat_sup_ar_q$Densidad*dat_sup_ar_q$Competencia, perm="Exact"))

```

En cuanto a Moricandia arvensis (Baza), los resultados de la permanova son: 

```{r}
 summary(lmp(dat_sup_ar_b$Sup_porc~dat_sup_ar_b$Suelo*dat_sup_ar_b$Densidad*dat_sup_ar_b$Competencia, perm="Exact"))
```

¿Qué pasa si añado "Localidad" como factor extra??
```{r}
summary(lmp(dat_sup_ar$`Emergencia_1`~dat_sup_ar$Localidad*dat_sup_ar$Densidad*dat_sup_ar$Competencia*dat_sup_ar$Suelo, perm="Exact"))
```

También lo podemos ver para Moricandia rytidocarpoides
```{r}
summary(lmp(dat_sup_ry$Sup_porc~dat_sup_ry$Suelo*dat_sup_ry$Densidad*dat_sup_ry$Competencia, perm="Exact"))

```

Y finalmente para Moricandia moricandioides 

```{r}
summary(lmp(dat_sup_mo$Sup_porc~dat_sup_mo$Suelo*dat_sup_mo$Densidad*dat_sup_mo$Competencia, perm="Exact"))
```



Para poder entender mejor los resultados del PERMANOVA podemos graficar los resulados
##Gráficos de emergencia 

PRIMERO LOS GRÁFICOS DE MORICANDIA ARVENSIS EN CONDICIONES DE QUESADA 
**Inciso= PROBLEMA, en competencia estamos poniendo dentro del mismo saco las dos especies que crecen mmmm

1. Emergencia Moricandia arvensis según el tipo de suelo


```{r}
summ30 <- summarySE(dat_sup_ar_q, measurevar = "Plant_emerg", groupvars=c("Suelo")) ##using function below
summ30<- summ30[-5,]
ggplot(data=summ30,aes(x=Suelo,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.3)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

2. Moricandia arvensis según el tipo de densidad

```{r}
summ31 <- summarySE(dat_sup_ar_q, measurevar = "Plant_emerg", groupvars=c("Densidad")) ##using function below
summ31<- summ31[-5,]
ggplot(data=summ31,aes(x=Densidad,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


3. Moricandia arvensis según el tipo de competencia
```{r}
summ32 <- summarySE(dat_sup_ar_q, measurevar = "Plant_emerg", groupvars=c("Competencia")) ##using function below
summ32<- summ32[-5,]
ggplot(data=summ32,aes(x=Competencia,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia M.arvensis& M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


4. Moricandia arvensis (Q) según el tipo de suelo y densidad

```{r}
summ33 <- summarySE(dat_sup_ar_q, measurevar = "Plant_emerg", groupvars=c("Suelo","Densidad")) ##using function below
summ33<- summ33[-5,]
ggplot(data=summ33,aes(x=Suelo,y=Plant_emerg,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Emergencia Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.20), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

5. Moricandia arvensis según tipo de suelo y competencia 

```{r}
summ34 <- summarySE(dat_sup_ar_q, measurevar = "Plant_emerg", groupvars=c("Suelo","Competencia")) ##using function below
summ34<- summ34[-5,]
ggplot(data=summ34,aes(x=Suelo,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia M.arvensis &M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.78,0.15), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

6.Emergencia Moricandia arvensis según la competencia y densidad 


```{r}
summ35 <- summarySE(dat_sup_ar_q, measurevar = "Plant_emerg", groupvars=c("Densidad","Competencia")) ##using function below
summ35<- summ35[-5,]
ggplot(data=summ35,aes(x=Densidad,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia M.arvensis & M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.25,0.18), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

MORICANDIA ARVENSIS EN CONDICIONES DE BAZA

7. Emergencia Moricandia arvensis(B) según el tipo de suelo


```{r}
summ36 <- summarySE(dat_sup_ar_b, measurevar = "Plant_emerg", groupvars=c("Suelo")) ##using function below
summ36<- summ36[-5,]
ggplot(data=summ36,aes(x=Suelo,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

8. Emergencia arvensis (B) según el tipo de densidad

```{r}
summ37 <- summarySE(dat_sup_ar_b, measurevar = "Plant_emerg", groupvars=c("Densidad")) ##using function below
summ37<- summ37[-5,]
ggplot(data=summ37,aes(x=Densidad,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


9. Moricandia arvensis(B) según el tipo de competencia
```{r}
summ38 <- summarySE(dat_sup_ar_b, measurevar = "Plant_emerg", groupvars=c("Competencia")) ##using function below
summ38<- summ38[-5,]
ggplot(data=summ38,aes(x=Competencia,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle("Emergencia M.arvensis &M.moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


10. Moricandia arvensis (B) según el tipo de suelo y densidad

```{r}
summ39 <- summarySE(dat_sup_ar_b, measurevar = "Plant_emerg", groupvars=c("Suelo","Densidad")) ##using function below
summ39<- summ39[-5,]
ggplot(data=summ39,aes(x=Suelo,y=Plant_emerg,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Emergencia de Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.80))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

11. Moricandia arvensis(B) según tipo de suelo y competencia 

```{r}
summ40 <- summarySE(dat_sup_ar_b, measurevar = "Plant_emerg", groupvars=c("Suelo","Competencia")) ##using function below
summ40<- summ40[-5,]
ggplot(data=summ40,aes(x=Suelo,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Emergencia M.arvensis &M.moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.75,0.80), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

12.Biomasa Moricandia arvensis(B) según la competencia y densidad 


```{r}
summ41 <- summarySE(dat_sup_ar_b, measurevar = "Plant_emerg", groupvars=c("Densidad","Competencia")) ##using function below
summ41<- summ41[-5,]
ggplot(data=summ41,aes(x=Densidad,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.33,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

GRÁFICOS DE MORICANDIA RYTIDOCARPOIDES

13. Emergencia Moricandia rytidocarpoides según el tipo de suelo


```{r}
summ42 <- summarySE(dat_sup_ry, measurevar = "Plant_emerg", groupvars=c("Suelo")) ##using function below
summ42<- summ42[-5,]
ggplot(data=summ42,aes(x=Suelo,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

14. Moricandia rytidocarpoides según el tipo de densidad

```{r}
summ43 <- summarySE(dat_sup_ry, measurevar = "Plant_emerg", groupvars=c("Densidad")) ##using function below
summ43<- summ43[-5,]
ggplot(data=summ43,aes(x=Densidad,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


15. Moricandia rytodcarpoides según el tipo de competencia
```{r}
summ44 <- summarySE(dat_sup_ry, measurevar = "Plant_emerg", groupvars=c("Competencia")) ##using function below
summ44<- summ44[-5,]
ggplot(data=summ44,aes(x=Competencia,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Emergencia /%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


16. Moricandia rtydocarpoides según el tipo de suelo y densidad

```{r}
summ45 <- summarySE(dat_sup_ry, measurevar = "Plant_emerg", groupvars=c("Suelo","Densidad")) ##using function below
summ45<- summ45[-5,]
ggplot(data=summ45,aes(x=Suelo,y=Plant_emerg,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Emergencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

 ```

17. Moricandia rytidocarpoides según tipo de suelo y competencia 

```{r}
summ46 <- summarySE(dat_sup_ry, measurevar = "Plant_emerg", groupvars=c("Suelo","Competencia")) ##using function below
summ46<- summ46[-5,]
ggplot(data=summ46,aes(x=Suelo,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.28,0.83))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```
18.Emergencia Moricandia rytidocarpoides según la competencia y densidad 


```{r}
summ47 <- summarySE(dat_sup_ry, measurevar = "Plant_emerg", groupvars=c("Densidad","Competencia")) ##using function below
summ47<- summ47[-5,]
ggplot(data=summ47,aes(x=Densidad,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.75,0.80),legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

GRÁFICOS DE MORICANDIA MORICANDIOIDES

19. Biomasa Moricandia moricandioides según el tipo de suelo


```{r}
summ48 <- summarySE(dat_sup_mo, measurevar = "Plant_emerg", groupvars=c("Suelo")) ##using function below
summ48<- summ48[-5,]
ggplot(data=summ48,aes(x=Suelo,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

20. Moricandia rytidocarpoides según el tipo de densidad

```{r}
summ49 <- summarySE(dat_sup_mo, measurevar = "Plant_emerg", groupvars=c("Densidad")) ##using function below
summ49<- summ49[-5,]
ggplot(data=summ49,aes(x=Densidad,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


21. Moricandia moricandioides según el tipo de competencia
```{r}
summ50 <- summarySE(dat_sup_mo, measurevar = "Plant_emerg", groupvars=c("Competencia")) ##using function below
summ50<- summ50[-5,]
ggplot(data=summ50,aes(x=Competencia,y=Plant_emerg))+
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespecífica", "Intraespecifica" = "Intraespecífica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


22. Moricandia moricandioides según el tipo de suelo y densidad

```{r}
summ51 <- summarySE(dat_sup_mo, measurevar = "Plant_emerg", groupvars=c("Suelo","Densidad")) ##using function below
summ51<- summ51[-5,]
ggplot(data=summ51,aes(x=Suelo,y=Plant_emerg,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Emergencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.30,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

23. Moricandia moricandioides según tipo de suelo y competencia 

```{r}
summ52 <- summarySE(dat_sup_mo, measurevar = "Plant_emerg", groupvars=c("Suelo","Competencia")) ##using function below
summ52<- summ52[-5,]
ggplot(data=summ52,aes(x=Suelo,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Suelo"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.28,0.23), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

24.Biomasa Moricandia moricandioides según la competencia y densidad 


```{r}
summ53 <- summarySE(dat_sup_mo, measurevar = "Plant_emerg", groupvars=c("Densidad","Competencia")) ##using function below
summ53<- summ53[-5,]
ggplot(data=summ53,aes(x=Densidad,y=Plant_emerg,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Emergencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Plant_emerg-se,ymax=Plant_emerg +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecífica", "Intraespecífica "))+
  labs(x=c("Densidad"),y=c("Emergencia (%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.22,0.86), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


#Gráficos de supervivencia 
 
1. Supervivencia Moricandia arvensis según el tipo de suelo


```{r}
summ54 <- summarySE(dat_sup_ar_q, measurevar = "Sup_porc", groupvars=c("Suelo")) ##using function below
summ54<- summ54[-5,]
ggplot(data=summ54,aes(x=Suelo,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia M.arvensis (Q)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.3)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

2. Moricandia arvensis según el tipo de densidad

```{r}
summ55 <- summarySE(dat_sup_ar_q, measurevar = "Sup_porc", groupvars=c("Densidad")) ##using function below
summ55<- summ55[-5,]
ggplot(data=summ55,aes(x=Densidad,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia  Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


3. Moricandia arvensis según el tipo de competencia
```{r}
summ56 <- summarySE(dat_sup_ar_q, measurevar = "Sup_porc", groupvars=c("Competencia")) ##using function below
summ56<- summ56[-5,]
ggplot(data=summ56,aes(x=Competencia,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia M.arvensis& M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


4. Moricandia arvensis (Q) según el tipo de suelo y densidad

```{r}
summ57 <- summarySE(dat_sup_ar_q, measurevar = "Sup_porc", groupvars=c("Suelo","Densidad")) ##using function below
summ57<- summ57[-5,]
ggplot(data=summ57,aes(x=Suelo,y=Sup_porc,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Supervivencia Moricandia arvensis (Q)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.50,0.20), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

5. Moricandia arvensis según tipo de suelo y competencia 

```{r}
summ58 <- summarySE(dat_sup_ar_q, measurevar = "Sup_porc", groupvars=c("Suelo","Competencia")) ##using function below
summ58<- summ58[-5,]
ggplot(data=summ58,aes(x=Suelo,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia M.arvensis &M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespec?fica", "Intraespec?fica "))+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.78,0.75), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

6.Emergencia Moricandia arvensis según la competencia y densidad 


```{r}
summ59 <- summarySE(dat_sup_ar_q, measurevar = "Sup_porc", groupvars=c("Densidad","Competencia")) ##using function below
summ59<- summ59[-5,]
ggplot(data=summ59,aes(x=Densidad,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia M.arvensis & M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespec?fica", "Intraespec?fica "))+
  labs(x=c("Densidad"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.55,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

MORICANDIA ARVENSIS EN CONDICIONES DE BAZA

7. Emergencia Moricandia arvensis(B) según el tipo de suelo


```{r}
summ60 <- summarySE(dat_sup_ar_b, measurevar = "Sup_porc", groupvars=c("Suelo")) ##using function below
summ60<- summ60[-5,]
ggplot(data=summ60,aes(x=Suelo,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

8. Emergencia arvensis (B) según el tipo de densidad

```{r}
summ61 <- summarySE(dat_sup_ar_b, measurevar = "Sup_porc", groupvars=c("Densidad")) ##using function below
summ61<- summ61[-5,]
ggplot(data=summ61,aes(x=Densidad,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


9. Moricandia arvensis(B) según el tipo de competencia
```{r}
summ62 <- summarySE(dat_sup_ar_b, measurevar = "Sup_porc", groupvars=c("Competencia")) ##using function below
summ62<- summ62[-5,]
ggplot(data=summ62,aes(x=Competencia,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle("Supervivencia M.arvensis &M.moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


10. Moricandia arvensis (B) según el tipo de suelo y densidad

```{r}
summ63 <- summarySE(dat_sup_ar_b, measurevar = "Sup_porc", groupvars=c("Suelo","Densidad")) ##using function below
summ63<- summ63[-5,]
ggplot(data=summ63,aes(x=Suelo,y=Sup_porc,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Supervivencia de Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.20))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

11. Moricandia arvensis(B) según tipo de suelo y competencia 

```{r}
summ64 <- summarySE(dat_sup_ar_b, measurevar = "Sup_porc", groupvars=c("Suelo","Competencia")) ##using function below
summ64<- summ64[-5,]
ggplot(data=summ64,aes(x=Suelo,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Supervivencia M.arvensis &M.moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespec?fica", "Intraespec?fica "))+
  labs(x=c("Suelo"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.75,0.18), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

12.Biomasa Moricandia arvensis(B) según la competencia y densidad 


```{r}
summ65 <- summarySE(dat_sup_ar_b, measurevar = "Sup_porc", groupvars=c("Densidad","Competencia")) ##using function below
summ65<- summ65[-5,]
ggplot(data=summ65,aes(x=Densidad,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia arvensis (B)")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespec?fica", "Intraespec?fica "))+
  labs(x=c("Densidad"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.50,0.47),legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

GRÁFICOS DE MORICANDIA RYTIDOCARPOIDES

13. Emergencia Moricandia rytidocarpoides según el tipo de suelo


```{r}
summ66 <- summarySE(dat_sup_ry, measurevar = "Sup_porc", groupvars=c("Suelo")) ##using function below
summ66<- summ66[-5,]
ggplot(data=summ66,aes(x=Suelo,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivenciaia M.rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

14. Moricandia rytidocarpoides según el tipo de densidad

```{r}
summ67 <- summarySE(dat_sup_ry, measurevar = "Sup_porc", groupvars=c("Densidad")) ##using function below
summ67<- summ67[-5,]
ggplot(data=summ67,aes(x=Densidad,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


15. Moricandia rytodcarpoides según el tipo de competencia
```{r}
summ68 <- summarySE(dat_sup_ry, measurevar = "Sup_porc", groupvars=c("Competencia")) ##using function below
summ68<- summ68[-5,]
ggplot(data=summ68,aes(x=Competencia,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


16. Moricandia rtydocarpoides según el tipo de suelo y densidad

```{r}
summ69 <- summarySE(dat_sup_ry, measurevar = "Sup_porc", groupvars=c("Suelo","Densidad")) ##using function below
summ69<- summ69[-5,]
ggplot(data=summ69,aes(x=Suelo,y=Sup_porc,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Supervivencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.15),legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

17. Moricandia rytidocarpoides según tipo de suelo y competencia 

```{r}
summ70 <- summarySE(dat_sup_ry, measurevar = "Sup_porc", groupvars=c("Suelo","Competencia")) ##using function below
summ70<- summ70[-5,]
ggplot(data=summ70,aes(x=Suelo,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  labs(x=c("Suelo"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.28,0.20),legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```
18.Emergencia Moricandia rytidocarpoides según la competencia y densidad 


```{r}
summ71 <- summarySE(dat_sup_ry, measurevar = "Sup_porc", groupvars=c("Densidad","Competencia")) ##using function below
summ71<- summ71[-5,]
ggplot(data=summ71,aes(x=Densidad,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia rytidocarpoides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  labs(x=c("Densidad"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.23,0.25),legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

GRÁFICOS DE MORICANDIA MORICANDIOIDES

19. Biomasa Moricandia moricandioides según el tipo de suelo


```{r}
summ72 <- summarySE(dat_sup_mo, measurevar = "Sup_porc", groupvars=c("Suelo")) ##using function below
summ72<- summ72[-5,]
ggplot(data=summ72,aes(x=Suelo,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

20. Moricandia rytidocarpoides según el tipo de densidad

```{r}
summ73 <- summarySE(dat_sup_mo, measurevar = "Sup_porc", groupvars=c("Densidad")) ##using function below
summ73<- summ73[-5,]
ggplot(data=summ73,aes(x=Densidad,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Densidad"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("Alta" = "Alta", "Baja" = "Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


21. Moricandia moricandioides según el tipo de competencia
```{r}
summ74 <- summarySE(dat_sup_mo, measurevar = "Sup_porc", groupvars=c("Competencia")) ##using function below
summ74<- summ74[-5,]
ggplot(data=summ74,aes(x=Competencia,y=Sup_porc))+
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  labs(x=c("Competencia"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


22. Moricandia moricandioides según el tipo de suelo y densidad

```{r}
summ75 <- summarySE(dat_sup_mo, measurevar = "Sup_porc", groupvars=c("Suelo","Densidad")) ##using function below
summ75<- summ75[-5,]
ggplot(data=summ75,aes(x=Suelo,y=Sup_porc,group=Densidad,colour=Densidad))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle("Supervivencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Densidad"),labels = c("Alta", "Baja "))+
  labs(x=c("Suelo"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.2)))+
  theme(legend.text  = element_text(size = rel(1.2)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.15,0.80))+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))

```

23. Moricandia moricandioides según tipo de suelo y competencia 

```{r}
summ76 <- summarySE(dat_sup_mo, measurevar = "Sup_porc", groupvars=c("Suelo","Competencia")) ##using function below
summ76<- summ76[-5,]
ggplot(data=summ76,aes(x=Suelo,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  labs(x=c("Suelo"),y=c("Supervivencia(%)"))+
  scale_x_discrete(labels=c("No" = "No perturbado", "Si" = "Perturbado"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1.1)))+
  theme(legend.text  = element_text(size = rel(1.1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.48,0.43), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

24.Biomasa Moricandia moricandioides según la competencia y densidad 


```{r}
summ77 <- summarySE(dat_sup_mo, measurevar = "Sup_porc", groupvars=c("Densidad","Competencia")) ##using function below
summ77<- summ77[-5,]
ggplot(data=summ77,aes(x=Densidad,y=Sup_porc,group=Competencia,colour=Competencia))+
  geom_line(size=1.5)+  
  geom_point(size=3)+
  ggtitle(" Supervivencia Moricandia moricandioides")+
  geom_errorbar(aes(ymin=Sup_porc-se,ymax=Sup_porc +se),width=0.1,position = position_dodge(0.01),size=1.5)+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Competencia"),labels = c("Interespecifica" = "Interespec?fica", "Intraespecifica" = "Intraespec?fica"))+
  labs(x=c("Densidad"),y=c("Supervivencia (%)"))+
  scale_x_discrete(labels=c("Alta","Baja"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(1.2)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.52,0.60), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


###DIFERENCIA ENTRE TRATAMIENTOS

#Diferencia Biomasa entre tratamientos M.arvensis(Q)

```{r}
class(dat_bm_ar_q$Tratamiento)
dat_bm_ar_q$Tratamiento<- as.factor(dat_bm_ar_q$Tratamiento)
mod3<- aovp(Biomasa_g~Tratamiento,data=dat_bm_ar_q)
summary(mod3)

library(multcomp)
Tukey.mod3<-glht(mod3, mcp(Tratamiento = "Tukey"))
summary(Tukey.mod3)
cld(Tukey.mod3,level=0.05,decreasing=F)
```


```{r}

ggplot(data=dat_bm_ar_q,aes(x=Tratamiento,y=Biomasa_g))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia arvensis Quesada")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Especie"),labels = c("M.rytidocarpoides"="M.rytidocarpoides", "M.arvensis"="M.arvensis(Q)"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  ylim(0,0.13)+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","RiAr2NP"="RiAr2NP","Ar10NP"="Ar10NP","RiAr10NP"="RiAr10NP","Ar2P"="Ar2P","RiAr2P"="RiAr2P","Ar10P"="Ar10P","RiAr10P"="RiAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


#Diferencia biomasa entre tratamientos M.arvensis (Baza)

```{r}
class(dat_bm_ar_b$Tratamiento)
dat_bm_ar_b$Tratamiento<- as.factor(dat_bm_ar_b$Tratamiento)

mod3<- aovp(Biomasa_g~Tratamiento,data=dat_bm_ar_b)
summary(mod3)

library(multcomp)
Tukey.mod3<-glht(mod3, mcp(Tratamiento = "Tukey"))
summary(Tukey.mod3)
cld(Tukey.mod3,level=0.05,decreasing=F)
```



```{r}
ggplot(data=dat_bm_ar_b,aes(x=Tratamiento,y=Biomasa_g))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia arvensis Baza")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.rytidocarpoides"="M.rytidocarpoides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  ylim(0,0.12)+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","MoAr2NP"="MoAr2NP","Ar10NP"="Ar10NP","MoAr10NP"="MoAr10NP","Ar2P"="Ar2P","MoAr2P"="MoAr2P","Ar10P"="Ar10P","MoAr10P"="MoAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

#Diferencia biomasa entre tratamientos M.rytidocarpoides

```{r}
class(dat_bm_ry$Tratamiento)
dat_bm_ry$Tratamiento<- as.factor(dat_bm_ry$Tratamiento)


mod3<- aovp(Biomasa_g~Tratamiento,data=dat_bm_ry)
summary(mod3)

library(multcomp)
Tukey.mod3<-glht(mod3, mcp(Tratamiento = "Tukey"))
summary(Tukey.mod3)
cld(Tukey.mod3,level=0.05,decreasing=F)
```

```{r}

ggplot(data=dat_bm_ry,aes(x=Tratamiento,y=Biomasa_g))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia rytidocarpoides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Especie"),labels = c("M.rytidocarpoides"="M.rytidocarpoides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","RiAr2NP"="RiAr2NP","Ar10NP"="Ar10NP","RiAr10NP"="RiAr10NP","Ar2P"="Ar2P","RiAr2P"="RiAr2P","Ar10P"="Ar10P","RiAr10P"="RiAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

#Diferencia biomasa entre tratamientos en M.moricandioides


```{r}
class(dat_bm_mo$Tratamiento)
dat_bm_mo$Tratamiento<- as.factor(dat_bm_mo$Tratamiento)

mod3<- aovp(Biomasa_g~Tratamiento,data=dat_bm_mo)
summary(mod3)

library(multcomp)
Tukey.mod3<-glht(mod3, mcp(Tratamiento = "Tukey"))
summary(Tukey.mod3)
cld(Tukey.mod3,level=0.05,decreasing=F)
```


```{r}

ggplot(data=dat_bm_mo,aes(x=Tratamiento,y=Biomasa_g))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia moricandioides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.moricandioides"="M.moricandioides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  ylim(0,0.010)+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","MoAr2NP"="MoAr2NP","Ar10NP"="Ar10NP","MoAr10NP"="MoAr10NP","Ar2P"="Ar2P","MoAr2P"="MoAr2P","Ar10P"="Ar10P","MoAr10P"="MoAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.42,0.19), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


#Diferencia competencia interespec?fica entre especies

Primero vamos a gacer subset seg?n la localidad 
```{r}
dat_bm_q<-subset(dat_bm,dat_bm$Localidad=="Quesada")

dat_bm_b<-subset(dat_bm,dat_bm$Localidad=="Baza")
```

QUESADA

```{r}

ggplot(data=dat_bm_q,aes(x=Tratamiento,y=Biomasa_g))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa M.arvensis y M.rytidocarpoides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.moricandioides"="M.moricandioides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  ylim(0,0.15)+
  scale_x_discrete(limits=c("RiAr2NP"="RiAr2NP","RiAr10NP"="RiAr10NP","RiAr2P"="RiAr2P","RiAr10P"="RiAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```

```{r}
ggplot(data=dat_bm_b,aes(x=Tratamiento,y=Biomasa_g))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia moricandioides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.moricandioides"="M.moricandioides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  ylim(0,0.1)+
  scale_x_discrete(limits=c("MoAr2NP"="MoAr2NP","MoAr10NP"="MoAr10NP","MoAr2P"="MoAr2P","MoAr10P"="MoAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.30,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


```{r}

ggplot(data=dat_bm_b,aes(x=Tratamiento,y=N_plantas))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia moricandioides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.moricandioides"="M.moricandioides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  scale_x_discrete(limits=c("MoAr2NP"="MoAr2NP","MoAr10NP"="MoAr10NP","MoAr2P"="MoAr2P","MoAr10P"="MoAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```



#Diferencia emergencia entre tratamientos

```{r}
dat_sup_q<-subset(dat_sup,dat_sup$Localidad=="Quesada")

dat_sup_b<-subset(dat_sup,dat_sup$Localidad=="Baza")
```


Tratamiento M.arv Quesada



```{r}

ggplot(data=dat_sup_ar_q,aes(x=Tratamiento,y=Plant_emerg))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Emergencia Moricandia arvensis Quesada")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Especie"),labels = c("M.rytidocarpoides"="M.rytidocarpoides", "M.arvensis"="M.arvensis(Q)"))+
  labs(x=c("Tratamiento"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","RiAr2NP"="RiAr2NP","Ar10NP"="Ar10NP","RiAr10NP"="RiAr10NP","Ar2P"="Ar2P","RiAr2P"="RiAr2P","Ar10P"="Ar10P","RiAr10P"="RiAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


```{r}
ggplot(data=dat_sup_ar_b,aes(x=Tratamiento,y=Plant_emerg))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Emergencia Moricandia arvensis Baza")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.moricandioides"="M.moricandioides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","MoAr2NP"="MoAr2NP","Ar10NP"="Ar10NP","MoAr10NP"="MoAr10NP","Ar2P"="Ar2P","MoAr2P"="MoAr2P","Ar10P"="Ar10P","MoAr10P"="MoAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


```{r}

ggplot(data=dat_sup_ry,aes(x=Tratamiento,y=Plant_emerg))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Emergencia Moricandia rytidocarpoides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Especie"),labels = c("M.rytidocarpoides"="M.rytidocarpoides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","RiAr2NP"="RiAr2NP","Ar10NP"="Ar10NP","RiAr10NP"="RiAr10NP","Ar2P"="Ar2P","RiAr2P"="RiAr2P","Ar10P"="Ar10P","RiAr10P"="RiAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.37,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```



```{r}
ggplot(data=dat_sup_mo,aes(x=Tratamiento,y=Plant_emerg))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Emergencia Moricandia moricandioides")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("M.arvensis(B)"),labels = c("M.moricandioides"="M.moricandioides", "M.arvensis"="M.arvensis"))+
  labs(x=c("Tratamiento"),y=c("Emergencia(%)"))+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","MoAr2NP"="MoAr2NP","Ar10NP"="Ar10NP","MoAr10NP"="MoAr10NP","Ar2P"="Ar2P","MoAr2P"="MoAr2P","Ar10P"="Ar10P","MoAr10P"="MoAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


#Diferencia supervivencia entre tratamientos(probando)


```{r}

ggplot(data=dat_sup_ar_q,aes(x=Tratamiento,y=Sup_porc))+
  geom_boxplot(aes(fill=Especie))+
  ggtitle(" Biomasa Moricandia arvensis Quesada")+
  scale_color_manual(values=c("red3","royalblue1"),name=c("Especie"),labels = c("M.rytidocarpoides"="M.rytidocarpoides", "M.arvensis"="M.arvensis(Q)"))+
  labs(x=c("Tratamiento"),y=c("Biomasa(g)"))+
  scale_x_discrete(labels=c("Ar2NP"="Ar2NP","RiAr2NP"="RiAr2NP","Ar10NP"="Ar10NP","RiAr10NP"="RiAr10NP","Ar2P"="Ar2P","RiAr2P"="RiAr2P","Ar10P"="Ar10P","RiAr10P"="RiAr10P"))+
  theme(axis.title = element_text(size = rel(1.5)))+
  theme(axis.text  = element_text(size = rel(0.8)))+
  theme(legend.title = element_text(size = rel(1)))+
  theme(legend.text  = element_text(size = rel(1)))+
  theme(axis.line = element_line(color = 'black'))+
  theme(legend.position=c(0.80,0.85), legend.background = element_blank())+
  theme( panel.grid.minor = element_blank()
         ,panel.background = element_rect(fill="white")
         ,panel.border = element_rect(fill=NA, color = "black"))
```


#Análisis de germinación (volver a hacer con aovp)

#M.arvensis Quesada

```{r}
modelo_suelo<- glm(Germ_b~Suelo, data = dat_sup_ar_q, family = 'binomial')
summary(modelo_suelo)

modelo_densidad<- glm(Germ_b~Densidad, data = dat_sup_ar_q, family = 'binomial')
summary(modelo_densidad)

modelo_competencia<- glm(Germ_b~Competencia, data = dat_sup_ar_q, family = 'binomial')
summary(modelo_competencia)

modelo<- glm(Germ_b~Suelo+Densidad+Competencia, data=dat_sup_ar_q,family="binomial")
summary(modelo)

modelo2<- glm(Germ_b~Suelo*Densidad+Competencia, data=dat_sup_ar_q,family="binomial")
summary(modelo2)
anova(modelo,modelo2)
modelo3<- glm(Germ_b~Suelo+Densidad*Competencia, data=dat_sup_ar_q,family="binomial")
summary(modelo3)

modelo4<- glm(Germ_b~Suelo*Densidad*Competencia, data=dat_sup_ar_q,family="binomial")
summary(modelo4)



```

#M.arvensis Baza

```{r}
modelo_suelo<- glm(Germ_b~Suelo, data = dat_sup_ar_b, family = 'binomial')
summary(modelo_suelo)

modelo_densidad<- glm(Germ_b~Densidad, data = dat_sup_ar_b, family = 'binomial')
summary(modelo_densidad)

modelo_competencia<- glm(Germ_b~Competencia, data = dat_sup_ar_b, family = 'binomial')
summary(modelo_competencia)

modelo<- glm(Germ_b~Suelo+Densidad+Competencia, data=dat_sup_ar_b,family="binomial")
summary(modelo)

modelo2<- glm(Germ_b~Suelo*Densidad+Competencia, data=dat_sup_ar_b,family="binomial")
summary(modelo2) #AIC más bajo

modelo3<- glm(Germ_b~Suelo+Densidad*Competencia, data=dat_sup_ar_b,family="binomial")
summary(modelo3)

modelo4<- glm(Germ_b~Suelo*Densidad*Competencia, data=dat_sup_ar_b,family="binomial")
summary(modelo4)



```


#M.rytidocarpoides
```{r}
modelo_suelo_ry<- glm(Germ_b~Suelo, data = dat_sup_ry, family = 'binomial')
summary(modelo_suelo_ry)

modelo_densidad_ry<- glm(Germ_b~Densidad, data = dat_sup_ry, family = 'binomial')
summary(modelo_densidad_ry)

modelo_competencia_ry<- glm(Germ_b~Competencia, data = dat_sup_ry, family = 'binomial')
summary(modelo_competencia_ry)

modelo_ry<- glm(Germ_b~Suelo+Densidad+Competencia, data=dat_sup_ry,family="binomial")
summary(modelo_ry)

modelo2_ry<- glm(Germ_b~Suelo*Densidad+Competencia, data=dat_sup_ry,family="binomial")
summary(modelo2_ry) 

modelo3_ry<- glm(Germ_b~Suelo+Densidad*Competencia, data=dat_sup_ry,family="binomial")
summary(modelo3_ry)

modelo4_ry<- glm(Germ_b~Suelo*Densidad*Competencia, data=dat_sup_ry,family="binomial")
summary(modelo4_ry)#AIC más bajo



```

#M.moricandioides

```{r}
modelo_suelo_mo<- glm(Germ_b~Suelo, data = dat_sup_mo, family = 'binomial')
summary(modelo_suelo_mo)

modelo_densidad_mo<- glm(Germ_b~Densidad, data = dat_sup_mo, family = 'binomial')
summary(modelo_densidad_mo)#AIC más bajo

modelo_competencia_mo<- glm(Germ_b~Competencia, data = dat_sup_mo, family = 'binomial')
summary(modelo_competencia_mo)

modelo_mo<- glm(Germ_b~Suelo+Densidad+Competencia, data=dat_sup_mo,family="binomial")
summary(modelo_mo)

modelo2_mo<- glm(Germ_b~Suelo*Densidad+Competencia, data=dat_sup_mo,family="binomial")
summary(modelo2_mo) 

modelo3_mo<- glm(Germ_b~Suelo+Densidad*Competencia, data=dat_sup_mo,family="binomial")
summary(modelo3_mo)

modelo4_mo<- glm(Germ_b~Suelo*Densidad*Competencia, data=dat_sup_mo,family="binomial")
summary(modelo4_mo)



```

###Curva de Supervivencia
Falta hacer la matriz para utilizar un script tal como :

#Analisis movimiento en campo


Datsu <-read.table("C:/Users/jrbr_/OneDrive/Documentos/Granada/Uni/TFM/2019_26_7/Liquenes_adhesivo_campo.txt", header=TRUE, sep="\t", na.strings="NA", dec=",", strip.white=TRUE)
names(Datsu)
str(Datsu)
library (survival)

#Movimiento liquenes EN CADA TRATAMIENTO
attach(Datsu)
status_h<-1*(died>0)

# Fit Kaplan-Meier survivor function
kmfit_h<-survfit(Surv(time,status_h)~TRes,type="kaplan-meier")
summary(kmfit_h)
kmfit_h

par(mar=c(5,5.5,2.5,1.5))

plot(kmfit_h,col=c("blue","goldenrod1","red", "orangered2","grey" ,"forestgreen"), main="Adherencia l?quenes",font.main=4,cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, lwd=3, 
     ylab="Adherencia",xlab="Muestreo",xlim=c(0,15),  ylim=c(0.5,1.0))
#legend(x = "bottomleft", c("Agua","Control","Cola","Goma","Hidro", "Resina"),col=c("blue","red","goldenrod1", "orangered2","grey" ,"forestgreen"),lwd=3,bty="n")
legend(x = "bottomleft", c("W","C","G","AG","HS", "SR"),col=c("blue","red","goldenrod1", "orangered2","grey" ,"forestgreen"),lwd=3,bty="n", horiz=T, cex=1.2)

#GLM
glm_campo<- glm(died~TRes, family=binomial, data=Datsu)
summary(glm_campo)

##Comparaciones por pares
install.packages("multcomp", dep=T)
library(multcomp)
glht.glm <- glht(glm_campo, mcp(TRes = "Tukey"))
summary(glht.glm)
cld(glht.glm,level=0.05,decreasing=F)

#Medias error st
library(plyr)
ddply(Datsu, .(TRes), summarise, N=length(died[!is.na(died)]),mean=mean(died[!is.na(died)]),sd=sd(died[!is.na(died)]),se=sd(died[!is.na(died)])/sqrt(length(died[!is.na(died)])))


#ANALISIS MOVIMIENTO SIMULACIONES,

#Simulacion 1

Datsu <-read.table("Simulacion_liquenes.txt",header=TRUE, sep="\t", na.strings="NA", dec=",", strip.white=TRUE)
Datsu$TRAT<-reorder(Datsu$TRAT,Datsu$SIMUL_1,mean)
library(sciplot)
bargraph.CI(TRAT,SIMUL_1, group = NULL, data = Datsu, main="Movimiento simulacion 1",font.main=4,xlab = "Tratamientos", ylab = "Movimiento",cex.names = 1.25,err.width=0.05,legend=T)

#GLM  
glm<-glm(SIMUL_1~TRAT,data=Datsu,family=binomial)
summary (glm)
glht.glm <- glht(glm, mcp(TRAT = "Tukey"))
summary(glht.glm)
cld(glht.glm,level=0.05,decreasing=F)

#Los resultados son iguales ya que no hubo movimiento en la simulacion 2 respecto a la 1
identical(Datsu$SIMUL_1,Datsu$SIMUL_2)
Datsu$SIMUL_1==Datsu$SIMUL_2

#Medias error st
library(plyr)
ddply(Datsu, .(TRAT), summarise,N=length( SIMUL_1[!is.na( SIMUL_1)]),mean=mean( SIMUL_1[!is.na( SIMUL_1)]),sd=sd( SIMUL_1[!is.na( SIMUL_1)]),se=sd( SIMUL_1[!is.na( SIMUL_1)])/sqrt(length( SIMUL_1[!is.na( SIMUL_1)])))

